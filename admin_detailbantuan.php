...
if (isset($_POST['update_bantuan'])) {
    ...
    if ($conn->query($sql_update) === TRUE) {
        ...
        echo "<script>alert('Data bantuan diperbarui!'); window.location='admin_detailbantuan.php?id=$id_bantuan';</script>";
    } else {
        echo "<script>alert('Gagal memperbarui data: " . $conn->error . "');</script>";
    }
}

// 4. PROSES ARSIP / AKTIFKAN KEMBALI
if (isset($_POST['arsip_bantuan'])) {
    // Ambil status saat ini untuk menentukan aksi
    $res = $conn->query("SELECT status FROM admin_programbantuan WHERE id_program = $id_bantuan");
    $row_status = $res->fetch_assoc();
    $status_baru = ($row_status['status'] == 'Arsip') ? 'Open' : 'Arsip';
    
    $sql_arsip = "UPDATE admin_programbantuan SET status = '$status_baru' WHERE id_program = $id_bantuan";
    if ($conn->query($sql_arsip) === TRUE) {
        $pesan = ($status_baru == 'Arsip') ? 'Program berhasil diarsipkan!' : 'Program berhasil diaktifkan kembali!';
        echo "<script>alert('$pesan'); window.location='admin_detailbantuan.php?id=$id_bantuan';</script>";
        exit;
    } else {
        echo "<script>alert('Gagal memperbarui status: " . $conn->error . "');</script>";
    }
}
...
$isArsip = (strtolower($data['status']) == 'arsip');

$v_c1 = (float)$data['c1_luas'];
$v_c2 = (float)$data['c2_penghasilan'];
$v_c3 = (float)$data['c3_komoditas'];
$v_c4 = (float)$data['c4_kondisi'];
$v_c5 = (float)$data['c5_alat'];
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .header-section { padding: 40px 50px 0 50px; flex-shrink: 0; }
        .scroll-section { padding: 0 50px 40px 50px; overflow-y: auto; flex-grow: 1; }
        ...
        .btn-arsip-action { background-color: <?= $isArsip ? '#0d7839' : '#6c757d' ?>; color: white; border: none; padding: 11px 22px; border-radius: 6px; font-size: 14px; font-weight: bold; cursor: pointer; display: flex; align-items: center; gap: 6px; }
        .btn-arsip-action:hover { opacity: 0.9; }
...
<body>
    ...
    <div class="dashboard-container">
        <div class="sidebar-brand-only">
            <div class="sidebar-logo-area">
                <div class="sidebar-logo-bg"><img src="logo.png" alt="Logo SIP-BANTU TANI" class="sidebar-logo-img"></div>
                ...
                <div class="form-box-container">
                    <form id="formBantuan" action="admin_detailbantuan.php?id=<?= $id_bantuan; ?>" method="POST">
                        ...
                        <div class="form-grid-2">
                            <div class="form-group"><label>Nama Program Bantuan</label><input type="text" name="nama_bantuan" value="<?= htmlspecialchars($data['nama_bantuan']); ?>" required disabled></div>
                            <div class="form-group"><label>Batas Akhir (Deadline)</label><input type="date" name="batas_akhir" value="<?= $data['batas_akhir']; ?>" required disabled></div>
                            <div class="form-group"><label>Status</label>
                                <select name="status" disabled>
                                    ...
                                    <option value="Arsip" <?= ($isArsip) ? 'selected' : ''; ?>>Arsip</option>
                                </select>
                            </div>
                            <div class="form-group"><label>Nama File Gambar</label><input type="text" name="gambar" value="<?= htmlspecialchars($data['gambar']); ?>" required disabled></div>
                        </div>
                        <div class="form-group"><label>Deskripsi Singkat (Opsional)</label><textarea name="deskripsi" rows="2" disabled><?= htmlspecialchars(!empty($data['deskripsi']) ? $data['deskripsi'] : "Program penyaluran bantuan resmi untuk mengoptimalkan hasil panen pertanian daerah."); ?></textarea></div>

                        <div class="form-section-title" style="margin-top: 15px;">Setting Bobot Kriteria Penilaian (Metode WP)</div>
                        <div class="form-grid-5">
                            <div class="form-group"><div class="kriteria-label"><label>C1: Luas Lahan</label><span class="badge-kriteria badge-benefit">Benefit</span></div><input type="number" class="input-bobot" name="c1_luas" step="0.01" min="0" max="1" value="<?= number_format($v_c1, 2); ?>" required disabled></div>
                            <div class="form-group"><div class="kriteria-label"><label>C2: Penghasilan</label><span class="badge-kriteria badge-cost">Cost</span></div><input type="number" class="input-bobot" name="c2_penghasilan" step="0.01" min="0" max="1" value="<?= number_format($v_c2, 2); ?>" required disabled></div>
                            <div class="form-group"><div class="kriteria-label"><label>C3: Komoditas</label><span class="badge-kriteria badge-benefit">Benefit</span></div><input type="number" class="input-bobot" name="c3_komoditas" step="0.01" min="0" max="1" value="<?= number_format($v_c3, 2); ?>" required disabled></div>
                            <div class="form-group"><div class="kriteria-label"><label>C4: Kondisi Lahan</label><span class="badge-kriteria badge-benefit">Benefit</span></div><input type="number" class="input-bobot" name="c4_kondisi" step="0.01" min="0" max="1" value="<?= number_format($v_c4, 2); ?>" required disabled></div>
                            <div class="form-group"><div class="kriteria-label"><label>C5: Alat Kerja</label><span class="badge-kriteria badge-cost">Cost</span></div><input type="number" class="input-bobot" name="c5_alat" step="0.01" min="0" max="1" value="<?= number_format($v_c5, 2); ?>" required disabled></div>
                        </div>

                        <div class="total-bobot-bar"><span>Total Alokasi Bobot Saat Ini:</span><span id="displayTotal" class="status-valid">0.00 (0%)</span></div>

                        <div class="action-footer-row">
                            <button type="submit" name="arsip_bantuan" class="btn-arsip-action" onclick="return confirm('Apakah Anda yakin ingin <?= $isArsip ? 'MENGAKTIFKAN KEMBALI' : 'MENGARSIPKAN' ?> program bantuan ini?')">
                                <span class="material-symbols-outlined" style="font-size: 18px;"><?= $isArsip ? 'unarchive' : 'archive' ?></span> <?= $isArsip ? 'Aktifkan Kembali' : 'Arsip' ?>
                            </button>
                            <div class="right-buttons">
                                <button type="button" id="btnEditToggle" class="btn-trigger-edit"><span class="material-symbols-outlined" style="font-size: 18px;">edit</span> Edit Data</button>
    ...
    <script>
        ...
        function hitungTotalBobot() {
            let total = 0;
            bobotInputs.forEach(input => total += parseFloat(input.value) || 0);
        ...
        hitungTotalBobot();
        bobotInputs.forEach(input => input.addEventListener('input', hitungTotalBobot));

        btnEditToggle.addEventListener('click', () => {
            isEditMode = !isEditMode;
            inputs.forEach(input => { if(input.name !== 'arsip_bantuan') input.disabled = !isEditMode; });
            btnEditToggle.innerHTML = isEditMode ? '<span class="material-symbols-outlined" style="font-size: 18px;">close</span> Batal' : '<span class="material-symbols-outlined" style="font-size: 18px;">edit</span> Edit Data';
            btnEditToggle.style.backgroundColor = isEditMode ? '#6c757d' : '#ffc107';
            btnSimpan.style.display = isEditMode ? 'block' : 'none';
        });
    </script>
</body>
</html>
